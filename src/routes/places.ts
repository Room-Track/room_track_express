...
router.delete('/', getQuerySpec('Places'), async (req: Request, res) => {
	...
	const { statusCode } = await deleteFamilyItem(client, 'Places', doc.id, doc);
...
router.post('/', getBodySpec('Places'), async (req: Request, res) => {
	const client: CosmosClient = req.app.get('client');
	...
	try {
		...
		for (const item of req.bodySpec.items as ModelType[]) {
			const result = await createFamilyItem(client, 'Places', item);
			results.push({
...
router.get('/', getQuerySpec('Places'), async (req: Request, res) => {
	const client: CosmosClient = req.app.get('client');
